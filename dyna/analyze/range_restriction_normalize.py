"""
Sound range-restriction normalization — the trustworthy sibling of `abbreviate`.

`abbreviate` (next door) is an aggressive type-specialization transform; its
projection decides "is this position pass-through" with the `$free` /
`ALLOW_FREE_MERGE` heuristic, whose soundness is asserted, not proven, and which
demonstrably drops the reflexive diagonal `path(I,I)` and computes all-zero
paths (`test_abbreviate.py::todo_startpath3`).

This module makes the opposite trade: project only what a *verifiable* condition
proves safe, and conservatively refuse the rest.  It has three layers:

  phantom_paths(program)
      The sound projectability analysis.  A position is a *phantom* iff, in
      every rule deriving the predicate, the head argument at that tree path is
      a variable that (1) occurs exactly once in the head [single-occurrence
      excludes diagonals: `path(I,I)` has the variable twice], (2) is in no
      test/builtin/constraint, and (3) has all body occurrences at phantom
      paths of literals matching the predicate's ground skeleton.  Soundness is
      an upward induction from base phantoms; it is keyed on tree *paths*, so
      wrapping a phantom in a functor (`g(w(X))`, `item(distraction, f(X))`)
      cannot hide it — the analysis is invariant under superficial structure.

  PhantomProjection(program, adom=None)
      Drops the phantom subterms, encoding the surrounding ground skeleton in a
      fresh functor (anti-unification) with skeleton-matched consumer rewriting
      and recovery rules; the summed-phantom multiplicity factor rides along.

  ValueSplit(program)
      Closes the constant/variable overlap gap (`f(1)`/`f(X)`) that the phantom
      analysis conservatively refuses, via a sound default+exception
      decomposition: f = f_default (general/phantom part, projected) + f_spec
      (specific deltas).  Sound by linearity of the semiring `+` over the rule
      partition — no disequality guards, no merge heuristic.

  RangeRestrictionNormalizer(program, adom=None)
      The entry point: ValueSplit then PhantomProjection.

Conservative by design: it refuses (soundly, leaving as residue) the diagonal
value-split (`q(X,X)`/`q(X,Y)`), recursive overlap, and nested value-splits.
Soundness is a rigorous argument, validated on startpath3 and thousands of
randomized differential programs, but not a mechanized proof.
"""
